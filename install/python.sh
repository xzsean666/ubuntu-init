...
export PATH="$HOME/.local/bin:$PATH"'
  ...
  append_user_file_if_missing "$TARGET_HOME/.bashrc" "ubuntu-bootstrap uv aliases" '# ubuntu-bootstrap uv aliases
alias pip="uv pip"
alias pipx="uv tool"'
...
export UV_DEFAULT_INDEX="https://pypi.tuna.tsinghua.edu.cn/simple"'
  ...
  if ! is_dry_run && run_user_shell 60 'export PATH="$HOME/.local/bin:$PATH"; command -v uv >/dev/null 2>&1 || [ -x "$HOME/.local/bin/uv" ]'; then
    ...
    skip_step "uv is already installed for $TARGET_USER. Aliases and configurations have been ensured."
    return $?
  fi
